...
rtx-spark-cli vendors its `.claude/skills/` from **guildmaster** — the
...
Every vendored `SKILL.md` carries `type: command`. rtx-spark-cli
...
#   - consumer-identifying prose: `guildmaster` → `rtx-spark-cli` (NOT
...
If a re-sync would lose a rtx-spark-cli adaptation, lift the change
