"""``rtx-spark-cli explain <path>...`` — global markdown catalog lookup (stable-contract).

``explain`` is global (not nested under a noun). It takes zero or more path
tokens and resolves them via the catalog in :mod:`rtx_spark_cli.explain`.
...
from rtx_spark_cli.explain import resolve
...
def register(sub: argparse._SubParsersAction) -> None:
    ...
    p.add_argument(
        "path",
        nargs="*",
        help="Command path tokens; empty = root (same as 'rtx-spark-cli').",
    )
