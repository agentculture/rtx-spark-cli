...
description: >
  ...
  defaults records to this agent's PERSONAL, PRIVATE scope (`--scope rtx-spark-cli
...
scope:** the default personal scope is **private** (`--scope rtx-spark-cli
...
- `--scope NAME` / `--visibility public|private` — record scope. **The wrapper
  ...
  (here, `rtx-spark-cli`). Private records are served only to a recall in the same
