...
type: command
description: >
  Cross-repo + mesh communication from rtx-spark-cli: file tracked GitHub issues
  ...
  the next step lives outside rtx-spark-cli (a brief for a sibling-repo agent, a
  status ping for a Culture channel, or pulling an issue body + comments
  into context). Issue posts auto-sign with `- rtx-spark-cli (Claude)`; mesh
  messages are unsigned (the IRC nick is the speaker). Not for in-rtx-spark-cli
