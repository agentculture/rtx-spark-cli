...
(`think`, `spec-to-plan`, `assign-to-workforce`) originate in `devague`, and
`outsource` originates in `convertible` — all re-broadcast via guildmaster.
...
`agtag issue`); **`convertible`** on PATH is an *optional* prerequisite the
`outsource` skill needs only when invoked (it exits with an install hint if
absent).
